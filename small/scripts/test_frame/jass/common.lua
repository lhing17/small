...
function jass.SetHeroLevel(h, level, showEyeCandy)
    log.debug('英雄' .. h:get_name() .. '的等级由' .. h:get_level() .. '设置为' .. level, '是否闪金光：', showEyeCandy)
    if h:is_hero() then
...
function jass.SuspendHeroXP(h, flag)
    if h:is_hero() then
...
function jass.GetUnitAbilityLevel(u, abilcode)
    return u and u:get_ability_level(abilcode) or 0
end
...
function jass.UnitItemInSlot(u, i)
    return u and u:get_item_in_slot(i) or nil
end
...
function jass.GetUnitState(u, whichUnitState)
    return u and u:get_unit_state(whichUnitState) or 0
end
...
function jass.IsPlayerInForce(p, f)
    if f == bj_FORCE_ALL_PLAYERS then
        return true
    end
...
function jass.SetPlayerState(p, whichPlayerState, value)
    if (whichPlayerState == PLAYER_STATE_RESOURCE_GOLD or whichPlayerState == PLAYER_STATE_RESOURCE_LUMBER) then
...
--- @param widget j_unit|j_item
--- @return number
--- FIXME
function jass.GetWidgetLife(widget)
    return 100.0
end
...
function jass.SetWidgetLife(widget, newLife)
    if widget.type == 'unit' then
        widget[unitstate[0]] = newLife
    elseif widget.type == 'item' then
        widget.life = newLife
    end
end
...
function jass.GetItemTypeId(it)
    return it and it:get_type_id() or 0
end
...
--- @param it j_item
function jass.GetItemCharges(it)
    return it.charge or 1
end
--native          SetItemCharges  takes item whichItem, integer charges returns nothing
--native          GetItemUserData takes item whichItem returns integer
function jass.GetItemUserData(it)
    return it and it:get_user_data() or 0
end
...
function jass.MultiboardGetTitleText()
    return lb and lb:get_title_text() or ""
end
...
function jass.PanCameraToTimed(x, y, duration)
    log.debug('将摄像机镜头x坐标设置为' .. x .. '，y坐标设置为:', y, '过渡时间为：', duration)
end
