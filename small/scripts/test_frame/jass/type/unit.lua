...
function mt:add_item(it)
    ...
    log.debug('为单位' .. self.handle_id .. '添加物品' .. it.handle_id)
...
function unit.create(p, unitid, x, y, face)
    --log.debug('为玩家' .. p.id .. '创建单位' .. unitid, '位置为：', x, y, '朝向为：', face)
    local u = {}
    setmetatable(u, unit)
    u.owner = p
    if p.color then
    ...
    u.abilities = {}
    u.items = {}
    u[unitstate[0]] = 100000000
    u[unitstate[1]] = 100000000
    u[unitstate[2]] = 100000000
    u[unitstate[3]] = 100000000
